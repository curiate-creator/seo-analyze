...
- Open Graph Title: "AI & Machine Learning Business Transformation Guide"`,
    ...
    try {
      ...
      setUrlAnalysis({
        ...
        performance: {
          expiresHeaders: {
            found: false,
            message: 'The server is not using "expires" headers for the images.',
          },
          jsMinified: {
            found: true,
          ...
          htmlSize: {
            size: "9 Kb",
            message: "The size of the HTML document is 9 Kb. This is under the average of 33 Kb.",
          },
          responseTime: {
            time: "0.2s",
  ...
  const insertSelectedKeywords = () => {
    ...
    keywordsToInsert.forEach((keyword, index) => {
      ...
      if (sentenceIndex < sentences.length && !sentences[sentenceIndex].toLowerCase().includes(keyword.toLowerCase())) {
    ...
    <div className=" bg-[#1E201E] flex items-center justify-center p-2 sm:h-dvh lg:min-h-screen">
      <div className="bg-[#3C3D37] rounded-lg shadow-lg overflow-hidden w-full max-w-5xl h-[calc(100dvh-1rem)] flex flex-col">
        ...
        <div className="bg-gradient-to-r from-[#3C3D37] to-purple-600 text-white p-2">
          ...
          <p className="text-sm opacity-90">Analyze your content and optimize it for better search rankings</p>
        </div>
        ...
        <div className="p-2 flex-1 overflow-y-auto">
          {activeTab === "analyze" && (
            <div className="space-y-2">
              <div>
                <label className="block text-xs font-medium text-[#ECDFCC] mb-1">Content to Analyze</label>
                <div className="relative">
                  <textarea
                    value={content}
              ...
              <div>
                <label className="block text-xs font-medium text-[#ECDFCC] mb-1">Target Keyword (Optional)</label>
                <input
                  type="text"
                  value={targetKeyword}
            ...
            <div className="space-y-2 animate-slideUp">
              ...
              <div className="bg-gradient-to-br from-blue-50 to-purple-50 rounded-lg p-3">
                <div className="flex items-center justify-between">
                  <div>
                    <h2 className="text-lg font-bold text-[#1E201E] mb-1">SEO Analysis Results</h2>
                    <p className="text-sm text-[#3C3D37]">
                      Grade: <span className="font-semibold">{analysis.seoAnalysis.grade}</span>
                    </p>
                  </div>
              ...
              {/* Detailed Breakdown */}
              <div className="grid md:grid-cols-2 gap-2 animate-slideUp" style={{ animationDelay: "0.1s" }}>
                <div className="bg-[#ECDFCC] border border-[#697565] rounded-lg p-3">
                  ...
                  <div className="space-y-2">
                    {Object.entries(analysis.seoAnalysis.breakdown).map(([category, data]) => (
                      <div key={category} className="flex items-center justify-between">
                        <div className="flex-1">
                          <div className="flex justify-between items-center mb-1">
                            <span className="text-xs font-medium text-[#1E201E]">{category}</span>
                          ...
                          <p className="text-xs text-[#3C3D37] mt-1">{data.reason}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="bg-[#ECDFCC] border border-[#697565] rounded-lg p-3">
                  ...
                  <div className="space-y-1">
                    {analysis.seoAnalysis.recommendations.map((rec, index) => (
                      <div key={index} className="flex items-start gap-2">
                        ...
                        <span className="text-xs text-[#1E201E]">{rec.replace("✓ ", "")}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                  {analysis.keywords.map((keyword, index) => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-medium text-[#1E201E]">{keyword.text}</span>
                        <span
                      ...
                      <div className="flex items-center justify-between mt-1">
                        <span className="text-xs text-[#3C3D37]">{Math.round(keyword.relevanceScore * 100)}%</span>
                        <span className="text-xs text-[#3C3D37]">x{keyword.frequency}</span>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                >
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                    <div className="text-center p-2 bg-[#697565] rounded">
                      <div className="text-lg font-bold text-[#1E201E]">{analysis.targetKeywordAnalysis.frequency}</div>
                      <div className="text-xs text-[#3C3D37]">Frequency</div>
                    </div>
                    <div className="text-center p-2 bg-[#697565] rounded">
                      <div className="text-lg font-bold text-[#1E201E]">{analysis.targetKeywordAnalysis.density}%</div>
                      <div className="text-xs text-[#3C3D37]">Density</div>
                    </div>
                    <div className="text-center p-2 bg-[#697565] rounded">
                      <div
                        className={`text-lg font-bold ${
                          analysis.targetKeywordAnalysis.inTitle ? "text-green-600" : "text-red-600"
                        }`}
                      >
                    ...
                    <div className="text-center p-2 bg-[#697565] rounded">
                      <div
                        className={`text-lg font-bold ${
                          analysis.targetKeywordAnalysis.firstParagraph ? "text-green-600" : "text-red-600"
                        }`}
                      >
              ...
              {/* Quick Actions */}
              <div className="flex flex-wrap gap-2 animate-slideUp" style={{ animationDelay: "0.5s" }}>
                <button
          ...
          {activeTab === "optimize" && analysis && (
            <div className="space-y-2 animate-slideUp">
              <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-lg p-3">
                ...
                <p className="text-sm text-[#3C3D37]">Choose how you want to optimize your content</p>
              </div>
                ...
                {[
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <h3 className="text-sm font-semibold text-[#1E201E] mb-1">{title}</h3>
              ...
              {optimization && (
                ...
                >
                  ...
                  <div className="mt-2 flex gap-2">
                    <button
                      onClick={() => navigator.clipboard.writeText(optimization.optimization)}
              ...
              {isOptimizing && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-center animate-slideUp">
                  ...
                  <p className="text-blue-800 text-sm font-medium">AI is optimizing your content...</p>
                </div>
              )}
            </div>
          )}
          ...
          {activeTab === "url-analysis" && (
            <div className="space-y-2 animate-slideUp">
              <div className="bg-gradient-to-br from-green-50 to-blue-50 rounded-lg p-3">
                ...
                <p className="text-sm text-[#3C3D37]">Analyze any website URL for technical SEO issues</p>
              </div>

              <div className="space-y-2">
                <div>
                  <label className="block text-xs font-medium text-[#ECDFCC] mb-1">Website URL to Analyze</label>
                  <input
                    type="url"
                    value={urlToAnalyze}
              ...
              {urlAnalysis && (
                <div className="space-y-2 animate-slideUp" style={{ animationDelay: "0.1s" }}>
                  ...
                  <div className="bg-[#ECDFCC] border border-[#697565] rounded-lg p-3 text-center">
                    ...
                    <p className="text-xs text-[#3C3D37] mt-1">Overall SEO Score</p>
                  </div>
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      {Object.entries(urlAnalysis.technicalSeo).map(([key, data]: [string, any], index) => (
                        <div
                          key={key}
                        ...
                        >
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-300 mt-1">{data.message}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  >
                    ...
                    <div className="grid md:grid-cols-2 gap-2">
                      {Object.entries(urlAnalysis.performance).map(([key, data]: [string, any], index) => (
                        <div
                          key={key}
                          className="flex items-start gap-2 p-2 bg-[#697565] rounded animate-slideUp"
                          style={{ animationDelay: `${0.5 + index * 0.05}s` }}
                        >
                          {data.found || data.time === "0.2s" || data.size === "9 Kb" ? (
                            <CheckCircle className="w-3 h-3 text-green-500 mt-0.5 flex-shrink-0" />
                          ) : (
                            <AlertCircle className="w-3 h-3 text-orange-500 mt-0.5 flex-shrink-0" />
                          )}
                          <div>
                            ...
                            <p className="text-xs text-gray-300">{data.message}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      {urlAnalysis.recommendations.map((rec: string, index: number) => (
                        <div
                          key={index}
                        ...
                        >
                          <AlertCircle className="w-3 h-3 text-orange-500 mt-0.5 flex-shrink-0" />
                          <span className="text-xs text-[#1E201E]">{rec}</span>
                        </div>
                      ))}
                    </div>
                  </div>
